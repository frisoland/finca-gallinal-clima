...
def render_sencrop_panel():
    ...
    st.caption("La app descarga los 4 sensores automáticamente y los combina en un único histórico.")
    ...
    if st.button("⬇️ Descargar e importar los 4 sensores de Sencrop", type="primary", use_container_width=True):
        ...
        if sensor_errors == ["TOKEN_EXPIRED"]:
            st.error(
                "🔑 El token de Sencrop ha caducado. "
                "Actualiza SENCROP_TOKEN en Secrets y pulsa el botón de abajo."
            )
            st.session_state.sencrop_token = None
        ...
        if base.empty:
            final = compact_history(df_sencrop)
        elif merge_mode == "Añadir solo datos nuevos":
            existing = set(pd.to_datetime(base["fecha_hora"]).dropna())
            only_new = df_sencrop[~pd.to_datetime(df_sencrop["fecha_hora"]).isin(existing)]
            final = compact_history(pd.concat([base, only_new], ignore_index=True))
        else:
            final = compact_history(pd.concat([base, df_sencrop], ignore_index=True))

        st.session_state.history_df     = final
        st.session_state.applied_period = None
        rng_min = pd.to_datetime(final["fecha_hora"]).min().date()
        rng_max = pd.to_datetime(final["fecha_hora"]).max().date()
        st.success(
